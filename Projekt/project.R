...
ui <- fluidPage(
  sidebarPanel(
    helpText("Author: Aleksandra Gomolka, 123034"),
    helpText("The app needs downloaded data. In the initial run please hit the DOWNLOAD button at the bottom of the sidebar."),
...
server <- function(input, output) {
  ...
  output$map <- renderPlot({
    map_data <- processed_data_all_countries() %>%
      ...
      mutate(country = replace(country, country == "Czechia", "Czech Rep."))
    
    eu_countries_map <- filtered_eu_countries %>%
      mutate(name = replace(name, name == "Czechia", "Czech Rep."))

    world_map <- ne_countries(scale = 50, returnclass = 'sf')

    european_union_map <- 
      world_map %>% 
      filter(name %in% eu_countries_map$name)
